...
import org.stellar.kotlinwalletsdk.utils.buildTransaction
import org.stellar.kotlinwalletsdk.utils.sponsorOperation
...
class Wallet(private val horizonUrl: String, private val networkPassphrase: String) {
  ...
  fun addAssetSupport(
    ...
    assetCode: String,
    assetIssuer: String,
    trustLimit: String = Long.MAX_VALUE.toBigDecimal().movePointLeft(7).toString(),
    sponsorAddress: String = ""
  ): Transaction {
    val isSponsored = sponsorAddress.isNotBlank()
  ...
  fun submitTransaction(signedTransaction: Transaction): Boolean {
    println("START")
    try {
  ...
  fun submitTransactionTest(signedTransaction: Transaction, mockServer: Server = server): Boolean {
    try {
      val response = mockServer.submitTransaction(signedTransaction)
